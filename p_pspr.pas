...
begin
  ...
  repeat
    if boolval(player.weaponowned[Ord(wp_plasma)]) and
       boolval(player.ammo[Ord(am_cell)]) and
...
begin
  ...
  begin
    damage := 5 * ((P_Random mod 3) + 1);
    angle := player.mo.angle;
    angle := angle + _SHLW(P_Random - P_Random, 19);  // VJ??? 19 = ANGLETOFINESHIFT
...
-----------------}
  ...
  begin
    ...
    P_SpawnMobj(
      linetarget.x, linetarget.y, linetarget.z + _SHR(linetarget.height, 2), MT_EXTRABFG);
