...
const
  ...
  LOADNET = 'you can''t do load while in a net game!' + #13#10 + #13#10 +
            PRESSKEY;
  QLOADNET = 'you can''t quickload during a netgame!' + #13#10 + #13#10 +
             PRESSKEY;
  QSAVESPOT = 'you haven''t picked a quicksave slot yet!' + #13#10 + #13#10 +
              PRESSKEY;
  SAVEDEAD = 'you can''t save if you aren''t playing!' + #13#10 + #13#10 +
             PRESSKEY;
  QSPROMPT = 'quicksave over your game named' + #13#10 + #13#10 +
             '''%s''?' + #13#10 + #13#10 +
             PRESSYN;
  QLPROMPT = 'do you want to quickload the game named' + #13#10 + #13#10 +
             '''%s''?' + #13#10 + #13#10 +
             PRESSYN;
  SNEWGAME = 'you can''t start a new game' + #13#10 +
             'while in a network game.' + #13#10 + #13#10 +
             PRESSKEY;
  SNIGHTMARE = 'are you sure? this skill level' + #13#10 +
               'isn''t even remotely fair.' + #13#10 +
               PRESSYN;
  SWSTRING = 'this is the shareware version of doom.' + #13#10 +
             'you need to order the entire trilogy.' + #13#10 +
             PRESSKEY;
  MSGOFF = 'Messages OFF';
  MSGON = 'Messages ON';
...
type
  endmsg_t = array[0..NUM_QUITMESSAGES - 1] of string;
  Pendmsg_t = ^endmsg_t;

var
  doom1_endmsg: endmsg_t;
  doom2_endmsg: endmsg_t;
...
initialization
  ...
  // DOOM1
  doom1_endmsg[0] := QUITMSG;
  doom1_endmsg[1] := 'please don''t leave, there''s more' + #13#10 + 'demons to toast!';
  doom1_endmsg[2] := 'let''s beat it -- this is turning' + #13#10 + 'into a bloodbath!';
  doom1_endmsg[3] := 'i wouldn''t leave if i were you.' + #13#10 + 'dos is much worse.';
  doom1_endmsg[4] := 'you''re trying to say you like dos' + #13#10 + 'better than me, right?';
  doom1_endmsg[5] := 'don''t leave yet -- there''s a' + #13#10 + 'demon around that corner!';
  doom1_endmsg[6] := 'ya know, next time you come in here' + #13#10 + 'i''m gonna toast ya.';
  doom1_endmsg[7] := 'go ahead and leave. see if i care.';

  // QuitDOOM II messages
  doom2_endmsg[0] := QUITMSG;
  doom2_endmsg[1] := 'you want to quit?' + #13#10 + 'then, thou hast lost an eighth!';
  doom2_endmsg[2] := 'don''t go now, there''s a ' + #13#10 + 'dimensional shambler waiting' + #13#10 + 'at the dos prompt!';
  doom2_endmsg[3] := 'get outta here and go back' + #13#10 + 'to your boring programs.';
  doom2_endmsg[4] := 'if i were your boss, i''d ' + #13#10 + ' deathmatch ya in a minute!';
  doom2_endmsg[5] := 'look, bud. you leave now' + #13#10 + 'and you forfeit your body count!';
  doom2_endmsg[6] := 'just leave. when you come' + #13#10 + 'back, i''ll be waiting with a bat.';
