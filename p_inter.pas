...
uses doomdef, dstrings, d_englsh, sounds, doomstat, m_rnd, i_system, am_map,
  p_local, p_mobj_h, s_sound, d_player;
...
begin
  ...
  case special.sprite of
    ...
    SPR_MGUN:
      begin
        if not P_GiveWeapon(player, wp_chaingun, boolval(special.flags and MF_DROPPED)) then
          exit;
    ...
    SPR_SHOT:
      begin
        if not P_GiveWeapon(player, wp_shotgun, boolval(special.flags and MF_DROPPED)) then
          exit;
    ...
    SPR_SGN2:
      begin
        if not P_GiveWeapon(player, wp_supershotgun, boolval(special.flags and MF_DROPPED)) then
          exit;
...
begin
  ...
  begin
    ...
    if boolval(target.player) then
      Pplayer_t(source.player).frags[pOperation(target.player, @players[0], '-', SizeOf(players[0]))] :=
        Pplayer_t(source.player).frags[pOperation(target.player, @players[0], '-', SizeOf(players[0]))] + 1;
  end
  ...
  begin
    ...
    if not boolval(source) then
      Pplayer_t(target.player).frags[pOperation(target.player, @players[0], '-', SizeOf(players[0]))] :=
        Pplayer_t(target.player).frags[pOperation(target.player, @players[0], '-', SizeOf(players[0]))] + 1;
...
begin	
  ...
  if boolval(inflictor) and (not boolval(target.flags and MF_NOCLIP)) and
    (not boolval(source) or (not boolval(source.player)) or (Pplayer_t(source.player).readyweapon <> wp_chainsaw)) then
  begin
  ...
  begin
    ...
    if (damage < 1000) and
       (boolval(player.cheats and CF_GODMODE) or boolval(player.powers[Ord(pw_invulnerability)])) then
      exit;
