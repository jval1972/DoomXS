...
begin
  ...
  begin
    ...
    while count > 0 do
    begin
      b := dc_colormap[dc_source[(LongWord(frac) div FRACUNIT) and 127]];
      dest^ := b;
  ...
  begin
    ...
    dest^ := dc_colormap[dc_source[(LongWord(frac) div FRACUNIT) and 127]];
    inc(dest, SCREENWIDTH);
    frac := frac + fracstep;
    dec(count);
  end;
...
begin
  ...
  begin
    // Re-map color indices from wall texture column
    //  using a lighting/special effects LUT.
    dest^ := dc_source[(LongWord(frac) div FRACUNIT) and 127]; // JVAL removed colormap, sky is full bright
...
begin
  ...
  begin
    ...
    dest^ := dc_colormap[dc_translation[dc_source[(LongWord(frac) div FRACUNIT) and 127]]];
    inc(dest, SCREENWIDTH);
